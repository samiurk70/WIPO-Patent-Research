...
def fig_a0_schematic():
    """Two filing routes and five counts. Square marks, one row, a count table."""
    fig, axes = plt.subplots(2, 1, figsize=(COL_2, 5.35),
                             gridspec_kw={"height_ratios": [1.15, 0.92], "hspace": 0.55})
    for ax in axes:
        ax.set_xlim(0, 10)
        ax.set_ylim(0, 10)
        ax.axis("off")
    ...
    def step(ax, x, text, face):
        width = 1.35
        ax.add_patch(Rectangle((x, 6.15), width, 1.35, facecolor=face,
                               edgecolor=INK, linewidth=0.6, zorder=2))
        ax.text(x + width / 2, 6.82, text, ha="center", va="center",
                fontsize=7.0, color=INK, zorder=3)

    def arrow(ax, x0, x1):
        ax.annotate("", xy=(x1, 6.82), xytext=(x0, 6.82),
                    arrowprops=dict(arrowstyle="-|>", color=INK, lw=0.7),
                    zorder=1)

    def count_table(ax, values):
        labels = ("Office-sum\nincl. **", "Office-sum\nexcl. **", "Leading\noffice",
                  "All families\n(6a)", "Foreign-\noriented (7)")
        xs = np.linspace(0.35, 8.15, 5)
        ax.plot([0.2, 9.7], [4.55, 4.55], color=INK, lw=0.6)
        ax.plot([0.2, 9.7], [2.15, 2.15], color=INK, lw=0.6)
        for x, lab, val in zip(xs, labels, values):
            ax.text(x + 0.7, 3.55, lab, ha="center", va="center", fontsize=6.6, color=INK)
            ax.text(x + 0.7, 2.7, str(val), ha="center", va="center",
                    fontsize=12, color=INK)

    ax = axes[0]
    ax.set_title("A   International route     one invention, not an average family",
                 loc="left", fontsize=8.5, color=INK, pad=1)
    ax.text(0.15, 8.55, "Priority", fontsize=7, color=INK)
    ax.text(3.35, 8.55, "+18 months", fontsize=7, color=INK)
    ax.text(6.7, 8.55, "+30 months", fontsize=7, color=INK)
    ax.plot([0.7, 8.3], [8.15, 8.15], color=INK, lw=0.7)
    for x in (0.7, 4.15, 7.55):
        ax.plot(x, 8.15, "o", color=INK, ms=3.2)
    # Home, PCT, three foreign offices, EPO: six publications.
    xs = [0.15, 1.7, 3.25, 4.8, 6.35, 7.9]
    labels = ("Home", "PCT **", "Foreign", "Foreign", "Foreign", "EPO")
    faces = ("#f7f6f3", "#f3e6d4", "#f7f6f3", "#f7f6f3", "#f7f6f3", "#e7e3f2")
    for x, lab, face in zip(xs, labels, faces):
        step(ax, x, lab, face)
    for i in range(5):
        arrow(ax, xs[i] + 1.35, xs[i + 1])
    count_table(ax, (6, 5, 1, 1, 1))

    ax = axes[1]
    ax.set_title("B   Domestic route     the same invention, filed only at home",
                 loc="left", fontsize=8.5, color=INK, pad=1)
    ax.text(0.15, 8.55, "Priority", fontsize=7, color=INK)
    ax.plot([0.7, 2.2], [8.15, 8.15], color=INK, lw=0.7)
    ax.plot(0.7, 8.15, "o", color=INK, ms=3.2)
    step(ax, 0.15, "Home", "#f7f6f3")
    ax.text(2.0, 6.82, "No PCT publication, and no foreign or regional publication.",
            ha="left", va="center", fontsize=7.5, color=MUTED)
    count_table(ax, (1, 1, 1, 1, 0))
